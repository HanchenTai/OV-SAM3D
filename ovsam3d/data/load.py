...
class SuperPoints:
    def __init__(self, super_points_path, top_num=200):
        with open(super_points_path) as f:
            super_points = json.load(f)
        super_points = np.array(super_points['segIndices'])
        seg_id, seg_inverse, seg_num = np.unique(super_points, return_inverse=True, return_counts=True)
        self.super_points_masks = np.zeros((seg_inverse.shape[0], seg_id.shape[0]), dtype=bool)
        self.super_points_masks[np.arange(seg_inverse.shape[0]), seg_inverse] = True
        self.top_index = np.argsort(-seg_num)[:top_num]
